...
def run_doctor(ai_config: AIConfig) -> list[dict[str, str]]:
    ...
    if ai_config.transport is None:
        checks.append(
            {"level": "error", "message": "ai.transport is required when enabled"},
        )
        return checks
    ...
    if ai_config.transport == AITransport.CLI:
        ...
        if binary is None:
            checks.append(
                {
                    "level": "error",
                    "message": f"Unknown CLI provider: {ai_config.provider}",
                },
            )
        elif not shutil.which(binary):
            checks.append(
                {"level": "error", "message": f"{binary} CLI not found on PATH"},
            )
        else:
        ...
        if env_var and not os.environ.get(env_var):
            checks.append(
                {"level": "error", "message": f"Set {env_var} for API transport"},
            )
        elif env_var:
    ...
    checks.append(
        {
            "level": "info",
            "message": (
                f"Provider: {ai_config.provider}, transport: {ai_config.transport}"
            ),
        },
    )
    return checks
